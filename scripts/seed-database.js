  ...
  try {
    ...
    envContent.split("\n").forEach((line) => {
      ...
      if (key && valueParts.length > 0) {
        ...
        if (cleanKey === "VITE_SUPABASE_URL") SUPABASE_URL = cleanValue;
        if (cleanKey === "VITE_SUPABASE_PUBLISHABLE_KEY") SUPABASE_KEY = cleanValue;
      }
    });
  } catch (e) {
...
if (!SUPABASE_URL || !SUPABASE_KEY) {
  ...
  console.error("   Make sure .env file is present with VITE_SUPABASE_URL and VITE_SUPABASE_PUBLISHABLE_KEY");
  process.exit(1);
}
...
async function seedDatabase() {
  try {
    ...
    if (heroResult.error) {
      console.warn("⚠️  Hero content:", heroResult.error.message);
    } else {
    ...
    if (eventResult.error) {
      console.warn("⚠️  Event spaces:", eventResult.error.message);
    } else {
    ...
    if (categoriesResult.error) {
      console.warn("⚠️  Menu categories:", categoriesResult.error.message);
    } else {
    ...
    const menus = [
      ...
      },
      {
        ...
        price: 50.0,
        category_id: categoryMap["เครื่องดื่ม"],
        image_url:
      ...
      },
      {
        ...
        price: 80.0,
        category_id: categoryMap["ของหวาน"],
        image_url:
    ...
    if (menusResult.error) {
      console.warn("⚠️  Menus:", menusResult.error.message);
    } else {
      console.log("✅ " + (menusResult.data?.length || 0) + " menus inserted");
    }
    console.log("");
    ...
    if (roomsResult.error) {
      console.warn("⚠️  Rooms:", roomsResult.error.message);
    } else {
      console.log("✅ " + (roomsResult.data?.length || 0) + " rooms inserted");
    }
    console.log("");
    ...
    if (roomsResult.data && roomsResult.data.length > 0) {
      ...
      if (roomImagesResult.error) {
        console.warn("⚠️  Room images:", roomImagesResult.error.message);
      } else {
    ...
    console.log("   ✅ Event spaces added");
    console.log("   ✅ Menu categories added");
    console.log("   ✅ Menus added (6 items)");
    console.log("   ✅ Rooms added (3 rooms)");
