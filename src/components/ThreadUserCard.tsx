...
export const ThreadUserCard = ({
  ...
  const hasCustomTitle = activePerks.includes("custom-title") && perksData?.custom_title;
  const hasFrame = activePerks.includes("custom-avatar-frame") && perksData?.avatar_frame;
  const frameClass = hasFrame ? AVATAR_FRAMES[perksData!.avatar_frame!]?.className || "" : "";

  if (compact) {
    return (
      <div className="flex items-center gap-2">
        <Avatar className={`w-8 h-8 ${frameClass}`}>
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-1">
            ...
            {hasCustomTitle && (
              <span className="text-[10px] text-blue-600 dark:text-blue-400">「{perksData!.custom_title}」</span>
            )}
          </div>
        </div>
      </div>
    ...
    <Card className={`overflow-hidden border-2 ${rank.borderColor} bg-gradient-to-br ${rank.bgColor}`}>
      <CardContent className="p-4">
        <div className="flex gap-4 mb-4">
          <Avatar className={`w-12 h-12 flex-shrink-0 border-2 border-white/20 ${frameClass}`}>
            {userAvatar && <img src={userAvatar} alt={userName} />}
