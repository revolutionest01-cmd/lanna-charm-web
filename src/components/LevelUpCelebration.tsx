          ...
          >
            <Card className={`border-4 ${toRank.borderColor} shadow-2xl max-w-md w-full mx-4 bg-gradient-to-br ${toRank.color} overflow-hidden`}>
              <div className="absolute inset-0 pointer-events-none overflow-hidden">
                {Array.from({ length: 20 }).map((_, i) => (
                  <motion.div
                    ...
                    style={{ background: ["#fbbf24", "#f59e0b", "#ec4899", "#8b5cf6", "#3b82f6"][i % 5] }}
                    initial={{ x: window.innerWidth / 2, y: window.innerHeight / 2, opacity: 1 }}
                    animate={{ x: window.innerWidth / 2 + (Math.random() - 0.5) * 400, y: window.innerHeight / 2 + (Math.random() - 0.5) * 400, opacity: 0, rotate: Math.random() * 360 }}
              ...
              <div className="relative z-10 p-8 text-center space-y-6">
                <motion.div className="flex justify-center" animate={{ y: [0, -10, 0] }} transition={{ duration: 1, repeat: Infinity }}>
                  <Trophy className="h-16 w-16 text-yellow-500" />
                </motion.div>

                <div className="space-y-2">
                  <h2 className="text-3xl font-bold font-serif">
                    {t4(language, "ยินดีด้วย! 🎉", "Congratulations! 🎉", "恭喜你！🎉", "おめでとうございます！🎉")}
                  </h2>
                  <p className="text-xl font-semibold">
                    {t4(language, "คุณได้เลื่อนขั้นเป็น", "You've been promoted to", "你已晋升为", "昇格しました")}
                  </p>
                </div>
                ...
                {newPerks.length > 0 && (
                  ...
                  >
                    <div className="flex items-center gap-2 justify-center font-semibold">
                      ...
                      {t4(language, "สิทธิพิเศษใหม่ที่ปลดล็อก", "New Perks Unlocked", "已解锁新特权", "新しい特典がアンロックされました")}
                    </div>
                ...
                <div className="space-y-1">
                  <p className="text-sm font-medium">
                    {t4(language, "สิทธิพิเศษทั้งหมด", "Total Perks", "全部特权", "全特典")}
                  </p>
                  <div className="flex justify-center gap-2 flex-wrap">
                    <Badge variant="secondary" className="gap-1">
                      <Sparkles className="h-3 w-3" />
                      {unlockedPerks.length} {t4(language, "รายการ", "items", "项", "件")}
                    </Badge>
                  </div>
                </div>
                ...
                <div className="flex gap-2 pt-4 justify-center">
                  <Button onClick={onClose} variant="default" className="gap-2">
                    {t4(language, "เข้าใจแล้ว!", "Got it!", "知道了！", "了解！")} <Sparkles className="h-4 w-4" />
                  </Button>
                </div>
