...
const QuestsAndAchievements = ({
  ...
  const totalDailyRewards = useMemo(() => {
    return DAILY_QUESTS.reduce((sum) => {
      return sum + 1; // each quest completed adds its reward
    }, 0);
  }, []);
    ...
    <div className="space-y-6">
            ...
            <div className="flex items-center justify-between">
              <div>
                <p className="text-xs text-muted-foreground">
                  {language === "th" ? "ภารกิจวันนี้" : "Today's Quests"}
                </p>
        ...
        <Card className="border-border/50 bg-gradient-to-br from-purple-50 to-purple-100 dark:from-purple-950/20 dark:to-purple-900/20">
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-xs text-muted-foreground">
                  {language === "th" ? "เกียรติยศ" : "Achievements"}
                </p>
        ...
        <Card className="border-border/50 bg-gradient-to-br from-amber-50 to-amber-100 dark:from-amber-950/20 dark:to-amber-900/20">
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-xs text-muted-foreground">
                  {language === "th" ? "รีเซ็ตใน" : "Reset In"}
                </p>
                <p className="text-2xl font-bold text-amber-600 dark:text-amber-400">
                  {timeUntilReset.hours}:{String(timeUntilReset.minutes).padStart(2, "0")}
                </p>
              </div>
            ...
            <CardContent className="space-y-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          <p className="font-semibold text-sm">
                            {language === "th" ? quest.name : quest.nameEn}
                          </p>
                          {isCompleted && (
                        ...
                        <div className="space-y-1">
                          <div className="flex justify-between items-center text-xs">
                            ...
                            <Badge className="bg-amber-600 text-white text-xs px-2 py-0">
                              +{quest.reward} {language === "th" ? "คะแนน" : "pts"}
                            </Badge>
                          </div>
                      ...
                      <div className="flex-shrink-0">
                        {isCompleted ? (
                          ...
                          <Button
                            size="sm"
                            variant="outline"
                            disabled={progress < quest.requirement}
                            className="disabled:opacity-50"
                          >
                            {language === "th" ? "อ้าง" : "Claim"}
                          </Button>
                        )}
                      </div>
                    </div>
              ...
              <div className="p-3 rounded-lg bg-amber-50 dark:bg-amber-950/20 border border-amber-200/50">
                <p className="text-xs text-amber-900 dark:text-amber-200">
                  ...
                  {language === "th"
                    ? "ภารกิจรายวันจะรีเซ็ตทุกวันเวลา 00:00 น. (เวลากรุงเทพ)"
                    : "Daily quests reset at 00:00 (Bangkok Time)"}
                </p>
              </div>
            </CardContent>
          </Card>
            ...
            <CardContent className="space-y-3">
              {ACHIEVEMENTS.map((achievement) => {
                ...
                const progressPercent = Math.min(
                  100,
                  Math.round((progress / (achievement.progress || 1)) * 100)
                );
                      ...
                      <div className="flex-1">
                        ...
                        {!isUnlocked && achievement.progress && (
                          <div className="space-y-1">
                            <div className="flex justify-between items-center text-xs">
                              <span className="text-muted-foreground">
                                {Math.min(progress, achievement.progress)}/{achievement.progress}
                              </span>
                              <Badge className="bg-purple-600 text-white text-xs px-2 py-0">
                                +{achievement.reward} {language === "th" ? "คะแนน" : "pts"}
                              </Badge>
                            </div>
                            <Progress value={progressPercent} className="h-2" />
                          </div>
              ...
              <div className="p-3 rounded-lg bg-gradient-to-r from-purple-100/50 to-purple-50/50 dark:from-purple-900/20 dark:to-purple-950/20 border border-purple-200/50">
                <p className="text-sm font-medium">
                  {language === "th"
                    ? `ได้เกียรติยศแล้ว ${totalAchievementCount}/${ACHIEVEMENTS.length}`
                    : `Achievements unlocked ${totalAchievementCount}/${ACHIEVEMENTS.length}`}
                </p>
              </div>
            </CardContent>
          </Card>
      ...
      <Card className="border-border/50 bg-blue-50/50 dark:bg-blue-950/20">
        ...
        <CardContent className="text-sm text-muted-foreground space-y-2">
          <p>
            • {language === "th"
              ? "สำเร็จภารกิจรายวันเพื่อเก็บคะแนนพิเศษ"
              : "Complete daily quests to earn bonus points"}
          </p>
          <p>
            • {language === "th"
              ? "ปลดล็อกเกียรติยศเพื่อได้ป้าย Badge พิเศษ"
              : "Unlock achievements to earn special badges"}
          </p>
          <p>
            • {language === "th"
              ? "มีเพียง 24 ชั่วโมงสำหรับสำเร็จภารกิจรายวัน"
              : "You have 24 hours to complete daily quests each day"}
          </p>
        </CardContent>
      </Card>
    </div>
